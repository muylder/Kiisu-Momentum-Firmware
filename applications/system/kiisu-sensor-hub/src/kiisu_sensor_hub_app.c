...
static void
    draw_accelerometer_summary(Canvas* canvas, int x, int y, int width, int height, void* data);
static void
    draw_magnetometer_summary(Canvas* canvas, int x, int y, int width, int height, void* data);
static void
    draw_temperature_summary(Canvas* canvas, int x, int y, int width, int height, void* data);
...
static void canvas_draw_xbm_rotated(
    Canvas* canvas,
    int cx,
    int cy,
    int w,
...
static void update_compass(App* app) {
    ...
    if(isnan(app->lsm_sample.mx) || isnan(app->lsm_sample.my) || isnan(app->lsm_sample.mz)) {
        return;
    }
    ...
    float axm, aym, azm;
    am_apply(app, ax, ay, app->lsm_sample.az, &axm, &aym, &azm);
    float mxm, mym, mzm;
    am_apply(app, mx, my, mz, &mxm, &mym, &mzm);
    ax = axm;
    ay = aym;
    mx = mxm;
    my = mym;
    ...
    float tilt_deg =
        acosf(fmaxf(-1.0f, fminf(1.0f, anz))) * 180.0f / (float)M_PI; // angle from gravity
    // Adjust alpha based on accel magnitude to reduce noise when motion detected
    float gmag = sqrtf(
        app->lsm_sample.ax * app->lsm_sample.ax + app->lsm_sample.ay * app->lsm_sample.ay +
        app->lsm_sample.az * app->lsm_sample.az);
    float a_dyn = a;
    ...
    if(hnorm > 1e-6f) {
        app->head_vec_x /= hnorm;
        app->head_vec_y /= hnorm;
    }
...
static void draw_calibrating_screen(Canvas* canvas, App* app) {
    ...
    static const uint8_t image_Little_Kiisu_0_bits[] = {
        };
    static const uint8_t image_Screenshot_2025_08_20_200213_0_bits[] = {
        ,
    ...
    if(app->calib_step == 0 || app->calib_step == 1) {
        ...
        if(app->calib_step == 1) {
            ...
            uint32_t remaining_ms =
                (elapsed >= app->calib_duration_ms) ? 0 : (app->calib_duration_ms - elapsed);
...
static void
    draw_humidity_summary(Canvas* canvas, int x, int y, int width, int height, void* data) {
    UNUSED(width);
    UNUSED(height);
...
static void draw_accelerometer_detail(Canvas* canvas, void* data) {
    ...
    if(app->accel_detail_page == 0) {
        ...
        if(sample->ok) {
            ...
            float sax, say, saz;
            am_apply(app, saxr, sayr, sazr, &sax, &say, &saz);
        ...
        static const uint8_t image_kitty_0_bits[] = {0x01, 0x08, 0x03, 0x0c, 0x07, 0x0e,
                                                     0x0f, 0x0f, 0xff, 0x0f, 0xff, 0x0f,
                                                     0xf3, 0x0c, 0xf3, 0x0c, 0xf3, 0x0c,
                                                     0xff, 0x0f, 0x0f, 0x0f, 0x9f, 0x0f,
                                                     0xff, 0x0f, 0xfe, 0x07, 0xfc, 0x03};
        for(int i = 0; i < app->sim_count; i++) {
            ...
            canvas_draw_xbm_rotated(
                canvas, cx, cy, app->sim_w, app->sim_h, image_kitty_0_bits, ang);
        }
        ...
        if(flat_mode) {
            ...
            if(th_deg <= 3.0f) {
                ...
                if(d > maxd && d > 1e-3f) {
                    bx = cx + (int)roundf(dx * (maxd / d));
                    by = cy + (int)roundf(dy * (maxd / d));
                }
            }
            ...
            if(!vertical_bar) {
                ...
                CanvasDirection dir = place_right ? CanvasDirectionTopToBottom :
                                                    CanvasDirectionBottomToTop;
                canvas_set_font_direction(canvas, dir);
                int nx = place_right ? (right_space_l + ((right_space_r - right_space_l) - 12) /
                                                            2) // 12px approx thickness
                                       :
                                       (left_space_l + ((left_space_r - left_space_l) - 12) / 2);
                // Center vertically by distributing string "length" (w) around the center line
                int ny = (dir == CanvasDirectionTopToBottom) ? (by + bh / 2 - w / 2) :
                                                               (by + bh / 2 + w / 2);
                if(adeg <= 3.0f) {
                    canvas_invert_color(canvas);
                    canvas_draw_box(canvas, nx - 3, ny - 12, w + 14, 20);
        ...
        if(sample->ok) {
            ...
            char obuf[32];
            snprintf(
                obuf, sizeof(obuf), "P:%+.0f", (double)(app->acc_pitch * 180.0f / (float)M_PI));
            int w = canvas_string_width(canvas, obuf);
            canvas_draw_str(canvas, 128 - w - 2, 14, obuf);
            snprintf(
                obuf, sizeof(obuf), "R:%+.0f", (double)(app->acc_roll * 180.0f / (float)M_PI));
...
static void draw_magnetometer_detail(Canvas* canvas, void* data) {
    ...
    if(app->mag_detail_page == 0) {
        ...
        float rmx, rmy, rmz;
        am_apply(app, rx, ry, rz, &rmx, &rmy, &rmz);
        if(app->sensor_swap_xy) {
            float t = rmx;
            rmx = rmy;
            rmy = t;
        ...
        snprintf(
            ...
            sizeof(buf),
            "Min  X:%+.1f Y:%+.1f Z:%+.1f",
            (double)app->mag_min_x,
            (double)app->mag_min_y,
            (double)app->mag_min_z);
        canvas_draw_str(canvas, 2, 24, buf);
        snprintf(
            buf,
            sizeof(buf),
            "Max  X:%+.1f Y:%+.1f Z:%+.1f",
            (double)app->mag_max_x,
            (double)app->mag_max_y,
            (double)app->mag_max_z);
...
static void draw_temperature_detail(Canvas* canvas, void* data) {
    ...
    static const uint8_t image_Ice_cube_0_bits[] = {
        ,
    ...
    // ST TEMP (LSM303)
    if(app->lsm_sample.ok && !isnan(app->lsm_sample.temp_c))
        snprintf(
            buf,
            sizeof(buf),
            "%.1f\xC2\xB0"
            "C",
            (double)app->lsm_sample.temp_c);
    else
        snprintf(
            buf,
            sizeof(buf),
    ...
    float t2 = (app->lsm_sample.ok && !isnan(app->lsm_sample.temp_c)) ? app->lsm_sample.temp_c :
                                                                        NAN;
    float avg = NAN;
...
static void draw_humidity_detail(Canvas* canvas, void* data) {
    ...
    // Icon bitmap (9x14)
    static const uint8_t image_icon_0_bits[] = {0x10, 0x00, 0x38, 0x00, 0x38, 0x00, 0x74,
                                                0x00, 0x74, 0x00, 0xfa, 0x00, 0xfa, 0x00,
                                                0xfd, 0x01, 0xff, 0x01, 0xff, 0x01, 0xff,
    ...
    char numbuf[8];
    if(sample->ok)
        snprintf(numbuf, sizeof(numbuf), "%.0f", (double)sample->humidity_rh);
    else
...
static void draw_light_detail(Canvas* canvas, void* data) {
    ...
    static const uint8_t image_pixil_frame_0__1__1_0_bits[] = {
        };
...
static void draw_cb(Canvas* canvas, void* ctx) {
    ...
    if(app->state == AppStateWidgetMenu) {
        ...
        static const uint8_t image_Thermometer_0_bits[] = {
            0xf8, 0x00, 0x8c, 0x01, 0x04, 0x01, 0x1c, 0x01, 0x04, 0x01, 0x04, 0x01, 0x1c,
            0x01, 0x04, 0x01, 0xfc, 0x01, 0xe4, 0x01, 0xfc, 0x01, 0xfc, 0x01, 0xe4, 0x01,
            0xfc, 0x01, 0xfc, 0x01, 0xe4, 0x01, 0xfc, 0x01, 0xfc, 0x01, 0xe4, 0x01, 0xfc,
            0x01, 0xfc, 0x01, 0xe4, 0x01, 0xfc, 0x01, 0xfc, 0x01, 0xe4, 0x01, 0xfc, 0x01,
            0xfc, 0x01, 0xfc, 0x01, 0xfe, 0x03, 0xff, 0x07, 0xff, 0x07, 0xff, 0x07, 0xff,
            0x07, 0xff, 0x07, 0xfe, 0x03, 0xfc, 0x01, 0xf8, 0x00};
        static const uint8_t image_icon_1_bits[] = {0x01, 0x00, 0x07, 0x00, 0x19, 0x00, 0xe7, 0xff,
                                                    0x1f, 0x80, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                                    0x1f, 0x00, 0x07, 0x00, 0x01, 0x00};
        static const uint8_t image_icon_2_bits[] = {0x10, 0x00, 0x38, 0x00, 0x38, 0x00, 0x74,
                                                    0x00, 0x74, 0x00, 0xfa, 0x00, 0xfa, 0x00,
                                                    0xfd, 0x01, 0xff, 0x01, 0xff, 0x01, 0xff,
                                                    0x01, 0xfe, 0x00, 0xfe, 0x00, 0x38, 0x00};
        static const uint8_t image_icon_3_bits[] = {0x07, 0x00, 0x00, 0x07, 0x00, 0x00, 0x07, 0x00,
                                                    0x00, 0x07, 0x00, 0x00, 0x07, 0x00, 0x00, 0x07,
                                                    0x00, 0x00, 0x07, 0x00, 0x00, 0x07};
        // Compass icon for the magnetometer box (16x15)
        static const uint8_t image_Compass_0_bits[] = {0xe0, 0x03, 0x58, 0x0f, 0x64, 0x1f,
                                                       0x3a, 0x3e, 0x3a, 0x3e, 0x3d, 0x7e,
                                                       0x3d, 0x7e, 0xbd, 0x7e, 0xff, 0x7f,
                                                       0x3f, 0x7e, 0xbe, 0x3f, 0x3e, 0x3e,
                                                       0xfc, 0x1e, 0x38, 0x0e, 0xe0, 0x03};
        ...
        if(app->home_has_kitty) {
            ...
            float ax = 0.0f, ay = 0.0f;
            if(app->lsm_sample.ok) {
                float saxr = app->lsm_sample.ax, sayr = app->lsm_sample.ay,
                      sazr = app->lsm_sample.az;
                if(app->sensor_swap_xy) {
                    float t = saxr;
                    saxr = sayr;
                    sayr = t;
            ...
            app->home_x += app->home_vx * dt;
            app->home_y += app->home_vy * dt;
            float yaw_spin = app->yaw_rate_dps * (float)M_PI / 180.0f * 0.5f;
        ...
        if(app->selected_widget >= 0 && app->selected_widget < WidgetTypeCount) {
            if(app->widgets[app->selected_widget].draw_detail) {
                app->widgets[app->selected_widget].draw_detail(
                    canvas, app->widgets[app->selected_widget].data);
            }
        }
                    ...
                    case InputKeyOk:
                        if(app->selected_widget == WidgetTypeMagnetometer &&
                           app->mag_detail_page == 1) {
                            ...
                            app->selected_widget == WidgetTypeAccelerometer &&
                            app->accel_detail_page == 2) {
        ...
        if(app->state == AppStateCalibrating) {
            if(app->calib_step == 1) {
                ...
                if(elapsed >= app->calib_duration_ms) {
                    ...
                    if(app->notification)
                        notification_message(app->notification, &sequence_semi_success);
                    if(app->calib_mode == CalibModeMagnetometer) {
                        // For magnetometer, proceed to figure-eight step
                        app->calib_duration_ms =
                            10000; // plan 10s for fig8 and display it on next page
