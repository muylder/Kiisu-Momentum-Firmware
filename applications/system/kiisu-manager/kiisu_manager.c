...
static void build_menu(App* app) {
    ...
    app->it_sleep =
        variable_item_list_add(list, "Time until sleep", DELAY_COUNT, sleep_changed, app);
    // Load from current local setting, default Always ON
    uint8_t idx = 0;
    for(uint8_t i = 0; i < DELAY_COUNT; i++) {
        if(delay_value[i] == app->sleep_delay_ms) {
            idx = i;
            break;
        }
    }
    ...
    app->it_brightness =
        variable_item_list_add(list, "LED Brightness", 21, brightness_changed, NULL);
...
int32_t kiisu_manager_app(void* p) {
    ...
    app->sleep_delay_ms = app->notification ? app->notification->settings.display_off_delay_ms :
                                              0; // default Always ON if missing
    ...
    if(!kiisu_settings_probe_ready()) {
        kiisu_ui_show_popup(
            app->ui, "Kiisu Manager", "Device @0x31 not found", 1500, popup_done, app);
